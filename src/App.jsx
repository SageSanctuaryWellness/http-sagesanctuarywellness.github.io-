...
import { Square, AlertCircle, ShieldCheck, Zap, ChevronRight, Volume2 } from 'lucide-react';
...
const App = () => {
  ...
  // Using Google Gemini API - Replace with your API key
  const apiKey = import.meta.env.VITE_GEMINI_API_KEY || "";

  // --- GOOGLE FONTS INJECTION ---
  useEffect(() => {
    const link = document.createElement('link');
    link.href = 'https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;700&family=Playfair+Display:ital@0;1&display=swap';
    link.rel = 'stylesheet';
  ...
  const analyzeSignal = async () => {
    ...
    const payload = {
      contents: [{ parts: [{ text: `Translate this operational signal for institutional leadership: ${input}` }] }],
    ...
    try {
      ...
      setAnalysis(data.candidates?.[0]?.content?.parts?.[0]?.text || 'No synthesis available.');
    } catch (err) {
  ...
  const generateForesight = async () => {
    ...
    try {
      const data = await fetchWithRetry(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${apiKey}`,
        { contents: [{ parts: [{ text: "Generate 3 foresight questions." }] }], systemInstruction: { parts: [{ text: systemPrompt }] } }
      );
      setForesight(data.candidates?.[0]?.content?.parts?.[0]?.text);
    } catch (err) {
  ...
  // --- FEATURE 3: AUDIO BRIEFING ---
  const playAudioBriefing = async () => {
    if (!analysis) return;
    setIsPlaying(true);

    const payload = {
      contents: [{ parts: [{ text: `Say in a calm, authoritative professional voice: ${analysis}` }] }],
      generationConfig: {
    ...
    <div className="min-h-screen bg-stone-50 py-16 px-6 flex flex-col items-center text-stone-900 font-sans">
      ...
      <main className="max-w-3xl w-full bg-white shadow-xl p-10 md:p-16 border border-stone-200 rounded-sm overflow-hidden">
        ...
        <section className="mb-12">
          <h2 className="text-4xl md:text-5xl font-serif leading-tight text-stone-900" style={{ fontFamily: '"Playfair Display", serif' }}>
            Translational Governance
          </h2>
          <p className="italic font-serif text-2xl md:text-3xl mt-4 text-stone-600" style={{ fontFamily: '"Playfair Display", serif' }}>
        ...
        <section className="mb-14 p-8 border border-stone-200 border-dashed bg-white">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="uppercase tracking-[0.2em] text-[11px] font-bold text-stone-500">Governance Foresight Generator</h3>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            ...
            <button
              ...
              className="px-6 py-3 border border-stone-900 text-stone-900 text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-stone-900 hover:text-white transition-all whitespace-nowrap"
            >
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-14 border-t pt-10">
          <div>
            ...
            <p className="text-xs leading-relaxed text-stone-500">Operational reality at the technical point of AI deployment and workforce interaction.</p>
          </div>
          <div>
            ...
            <p className="text-xs leading-relaxed text-stone-900 font-medium">
              Translating raw signals into governance foresight and senior leadership decision support.
            </p>
          </div>
          <div>
            <p className="uppercase text-[11px] tracking-[0.25em] text-stone-400 font-bold mb-3">Community</p>
            <p className="text-xs leading-relaxed text-stone-500">
              Systemic accountability, long-term equity, and maintaining enduring institutional trust.
            </p>
          </div>
        </section>
        ...
        <footer className="border-t pt-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
          <div>
            <p className="font-bold text-sm uppercase tracking-widest text-stone-800">Nadjean Sagesse</p>
        ...
        {error && (
          <div className="fixed bottom-6 right-6 bg-stone-900 text-white p-5 rounded-sm shadow-2xl flex items-center gap-4 text-xs border border-stone-700">
            <AlertCircle size={18} className="text-amber-500" />
            <div className="space-y-1">
              <p className="font-bold uppercase tracking-widest">Connectivity Signal</p>
